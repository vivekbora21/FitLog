...
Thresholds follow the recovery pillar in pacing.py (sleep 7.5h / 6.5h, steps
8,000 / 6,000). Volume targets (workouts, cardio) are pro-rated by the days
already finished — today is still in progress — so an untouched Monday isn't red.
"""
import math
...
STEPS_TARGET, STEPS_WARN = 8000, 6000
SLEEP_TARGET, SLEEP_WARN = 7.5, 6.5
...
def _build_metrics(stats, targets, days_elapsed, days_in_week):
    ...
    if avg_steps is None:
        ...
        steps_status = GOOD if avg_steps >= STEPS_TARGET else WARN if avg_steps >= STEPS_WARN else BAD

    if avg_sleep is None:
        sleep_status = PENDING
    else:
        sleep_status = GOOD if avg_sleep >= SLEEP_TARGET else WARN if avg_sleep >= SLEEP_WARN else BAD
    ...
    return [
        ...
        _metric('sleep', 'Sleep', avg_sleep, SLEEP_TARGET, 'h', sleep_status,
                f"sleep — averaging {avg_sleep}h, aim for {SLEEP_TARGET}–8.5h"
...
def build_weekly_health(user, weekly_review, weekly_workouts_target, cardio_target,
                        calories_target, protein_target, today):
