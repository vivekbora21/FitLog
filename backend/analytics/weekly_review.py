...
def _personal_note(rule, target_kcal, avg_calories, avg_steps, avg_sleep):
    ...
    if rule == 'Rule 3':
        ...
        if avg_steps is not None and avg_steps < 8000:
            note += f", or lift steps from {avg_steps:,}/day toward 8,000–10,000"
        return note + "."

    if rule == 'Rule 5':
        causes = []
        if avg_sleep is not None and avg_sleep < 7:
            causes.append(f"sleep averaged {avg_sleep}h (target 7.5–8.5h)")
...
def build_weekly_review(user, program, start_weight, start_waist, weekly_workouts_target, target_kcal, today):
